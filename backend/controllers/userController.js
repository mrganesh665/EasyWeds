...
export const signUp = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "OTP sent successfully", full_name, otp: otp });
  } catch (error) {
...
export const verifySignUpOtp = async (req, res) => {
  ...
  try {
    ...
    const username = `user${phone.replace("+", "")}`; // e.g., +91000000003 -> user91000000003
...
export const verifyVendorOtp = async (req, res) => {
  ...
  try {
    ...
    const username = `user${phone.replace("+", "")}`; // e.g., +91000000003 -> user923000000003
