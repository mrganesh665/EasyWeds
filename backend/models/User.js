...
const userSchema = new mongoose.Schema(
  {
    phone: {
      ...
      unique: true,
      match: [/^\+91[6-9]\d{9}$/, "Phone must be a valid Indian mobile number starting with +91"],
    },
    username: {
      type: String,
    ...
    vendorRequest: {
      ...
      phone_whatsapp: { type: String, match: [/^\+91[6-9]\d{9}$/, "Valid number"] },
      whatsapp_number: { type: String, match: [/^\+91[6-9]\d{9}$/, "Valid number"]  },
      category: {
        type: String,
        enum: [
          "Wedding Venues",
